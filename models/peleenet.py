...
class _DenseLayer(nn.Module):
    def __init__(self, num_input_features, growth_rate, bottleneck_width, drop_rate):
        ...
        inter_channel = int(growth_rate * bottleneck_width / 4) * 4  # 瓶颈层通道数目
...
class _StemBlock(nn.Module):  # Modification : stride of stem1 from 2 to 1
    def __init__(self, num_input_channels, num_init_features):
        super(_StemBlock, self).__init__()

        num_stem_features = int(num_init_features/2)

        self.stem1 = BasicConv2d(num_input_channels, num_init_features, kernel_size=3, stride=1, padding=1)
    ...
    def forward(self, x):
        out_stem1 = self.stem1(x)

        branch2 = self.stem2a(out_stem1)
        branch2 = self.stem2b(branch2)
        branch1 = self.pool(out_stem1)

        out = torch.cat([branch1, branch2], 1)
        out = self.stem3(out)

        return out_stem1, out
...
class PeleeNet(nn.Module):
    ...
    def __init__(self, growth_rate=32, block_config=[3, 4, 8, 6],
        ...
        self.pool = [1, 2]  # 含pool2d的stage(Sum : 4)
        self.flags = [False, True, True, False, False]  # 需要输出中间特征图的stage(Sum : 5)
        for i, num_layers in enumerate(block_config):
            ...
            layers = []
            layers.append(block)
            layers.append(BasicConv2d(num_features, num_features, kernel_size=1, stride=1, padding=0))
            if i in self.pool:
                layers.append(nn.AvgPool2d(kernel_size=2, stride=2))
            self.features.add_module('stage%d' % (i + 1), nn.Sequential(*layers))
...
def peleenet(**kwargs):
    ...
    """
    model = PeleeNet(growth_rate=32, block_config=[3, 4, 8, 6], num_init_features=32,
                     bottleneck_width=[1, 2, 4, 4], **kwargs)
