...
def get_scheduler(optimizer, args):
    ...
    if args.lr_policy == 'linear':
        def lambda_rule(epoch):
            ...
            return lr_l
        scheduler = lr_scheduler.LambdaLR(optimizer, lr_lambda=lambda_rule)  # lr_lambda参数应为计算lambda数值的函数或函数List
...
class ResNet(torch.nn.Module):
    def __init__(self, input_nc, output_nc,
        ...
        if self.resnet_stages_num == 5:
            ...
            raise NotImplementedError
        self.conv_pred = nn.Conv2d(layers, 32, kernel_size=3, padding=1)   # CNN BackBone 输出通道数为32

        self.output_sigmoid = output_sigmoid
        self.sigmoid = nn.Sigmoid()

    def forward(self, x):  # forward : neglect the TE and TD
        x1 = self.forward_single(x)
        x2 = self.forward_single(x)
        x = torch.abs(x1 - x2)
        if not self.if_upsample_2x:
            x = self.upsamplex2(x)
        x = self.upsamplex2(x)   # exists when input_size = 1024
        x = self.upsamplex4(x)
        x = self.classifier(x)
    ...
    def forward_single(self, x):  # CNN Backbone to produce two feature maps
        # resnet layers
        x = self.resnet.conv1(x)
        x = self.resnet.bn1(x)
        x = self.resnet.relu(x)
        x = self.resnet.maxpool(x)  # 1/4,out=64

        x_4 = self.resnet.layer1(x)  # 1/4, in=64, out=64
        x_8 = self.resnet.layer2(x_4)  # 1/8, in=64, out=128

        if self.resnet_stages_num > 3:
            x_8 = self.resnet.layer3(x_8)  # in=128, out=256

        if self.resnet_stages_num == 5:
            x_8 = self.resnet.layer4(x_8)  # in=256, out=512
...
class BASE_Transformer(ResNet):
    ...
    def __init__(self, input_nc, output_nc, with_pos, resnet_stages_num=5,
        ...
        decoder_pos_size = 1024 // 8  # the size for the output of the CNN Backbone
...
if __name__ == "__main__":
    ...
    net = BASE_Transformer(input_nc=3, output_nc=2, token_len=4, resnet_stages_num=4,
                           with_pos='learned', dim_head=8, decoder_dim_head=8, enc_depth=1, dec_depth=8)
    # net = ResNet(input_nc=3, output_nc=2, resnet_stages_num=4, output_sigmoid=False)
    net = init_net(net, init_type='normal', init_gain=0.02, gpu_ids=[1])
    # summary(net, input_size=(3, 1024, 1024), batch_size=8)
    # stat(net,(3,256,256))
    # print(sum(m.numel() for m in net.transformer_1.parameters()))
    # print(sum(m.numel() for m in net.transformer_2.parameters()))
    # print(sum(m.numel() for m in net.transformer_3.parameters()))
    # print(sum(m.numel() for m in net.transformer_4.parameters()))
