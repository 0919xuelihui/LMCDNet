...
transform_tensorboard = transforms.Compose([
                                transforms.Resize(256),  # 不这样作的话，获得的feature map尺寸越小，显示效果很差
                                transforms.ToTensor()
                                ])
...
def visulize_features(features, normalize=False):
    ...
    grid = make_grid(features)   # default value of row : 8, size of output : 1, h*b*c/row, w*row
...
def visualize_tensors(*tensors):
    ...
    :param tensors: tensor: C*H*W, C=1 or 3
    :return:
    """
    import matplotlib.pyplot as plt
...
def save_vis_features(features, layer_name, img_name, save_dir='./feature_map'):
    features = features.cpu().numpy()
    check_dir(save_dir)
    save_change_map_dir_ = os.path.join(save_dir, layer_name)
    ...
    for i in range(channel):
        ...
        pmin = np.min(img)
        pmax = np.max(img)
        img = ((img - pmin) / (pmax - pmin + 0.000001)) * 255  # Normalize
        img = img.astype(np.uint8)
        img = cv2.applyColorMap(img, cv2.COLORMAP_JET)
        img = img[:, :, ::-1]   # cv2(BGR)和 Image(RGB)通道是相反的
        img = Image.fromarray(img)
        # img = img.resize((256, 256), Image.NEAREST)
        save_fig_dir = os.path.join(save_change_map_dir_, img_name + 'channel_' + str(i) + '.jpg')
        img.save(save_fig_dir)


def save_DA_vis(features, save_dir='./feature_map/DA_vis'):
    min = torch.min(features, dim=1, keepdim=True)[0]
    max = torch.max(features, dim=1, keepdim=True)[0]
    features = ((features - min) / (max - min + 0.000001)) * 255
    features = features.cpu().numpy()
    # tmp = max - min
    # print(tmp[0, 0, 66, 10:20, 10:20])
    # print(features[0, 0, 66, 10:20, 10:20])
    # print(features[0, 1, 66, 10:20, 10:20])
    # print(features[0, 2, 66, 10:20, 10:20])
    # print(features[0, 3, 66, 10:20, 10:20])
    check_dir(save_dir)
    channel = features.shape[2]
    for j in range(4):
        imgs = features[0, j, :, :, :]
        save_change_map_dir_ = os.path.join(save_dir, str(j))
        check_dir(save_change_map_dir_)
        for i in range(channel):
            img = imgs[i, :, :]
            img = img.astype(np.uint8)
            ...
            img = Image.fromarray(img)
            # img = img.resize((256, 256), Image.NEAREST)
            save_fig_dir = os.path.join(save_change_map_dir_, 'channel_' + str(i) + '.jpg')
            img.save(save_fig_dir)
